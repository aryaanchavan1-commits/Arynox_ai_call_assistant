...
import { OpenCodeConversationResponder } from '../src/opencode-conversation-responder.js';
import { OpenAiConversationResponder } from '../src/openai-conversation-responder.js';

const E164_RE = /^\+[1-9]\d{7,14}$/;
const PROVIDERS = new Set(['groq', 'opencode', 'openai']);
...
const requestedProvider = (process.env.AGENTCALL_BRAIN_PROVIDER || '').trim().toLowerCase();
const callerName = process.env.AGENTCALL_QUALIFICATION_CALLER_NAME || 'Friend';
...
if (requestedProvider && !PROVIDERS.has(requestedProvider)) {
  throw new Error('AGENTCALL_BRAIN_PROVIDER must be groq, opencode, or openai');
}
...
async function keyFromProviderSettings(provider) {
  let settings;
  try {
    settings = JSON.parse(await readFile(providerSettingsPath, 'utf8'));
  } catch {
    return null;
  }
  const candidates = [settings?.stt, settings?.tts];
  const entry = candidates.find((value) => value?.provider === provider && typeof value.apiKey === 'string');
  return entry?.apiKey ?? null;
}

async function resolveBrain() {
  const provider = requestedProvider || (process.env.OPENCODE_API_KEY ? 'opencode' : 'groq');
  const model = process.env.AGENTCALL_CONVERSATION_MODEL || DEFAULT_MODELS[provider];
  let apiKey = process.env[ENV_KEYS[provider]];
  if (!apiKey) apiKey = await keyFromProviderSettings(provider);
  if (!apiKey) {
    throw new Error(`no API key found for ${provider}; set ${ENV_KEYS[provider]} or save it in Speech settings`);
  }
  if (provider === 'groq') return { provider, model, responder: new GroqConversationResponder({ apiKey, model }) };
  if (provider === 'opencode') return { provider, model, responder: new OpenCodeConversationResponder({ apiKey, model }) };
  return { provider, model, responder: new OpenAiConversationResponder({ apiKey, model }) };
}
...
async function main() {
  ...
  const responder = brain.responder;
  ...
  try {
    ...
    if (status.currentCall?.phase === 'active' && status.currentCall.direction === 'outgoing') {
      ...
      write({ phase: 'attached', callId, brain: `${brain.provider}/${brain.model}` });
    } else {
      ...
      const receipt = await client.dial({
        ...
        consent: {
          recorded: true,
          policy: 'explicit operator approval for AI assisted call',
        },
        destination: phoneNumber,
        idempotencyKey: `ai-call-${randomUUID()}`,
      });
      if (receipt?.accepted !== true) {
        throw new Error(`dial refused: ${receipt?.reason || 'unknown'}`);
      }
      callId = (await dialingEvent).callId;
      write({ phase: 'dialing', callId, brain: `${brain.provider}/${brain.model}` });
    ...
    if (!skipGreeting) {
      const greeting = `Hello ${callerName}, this is Arynox. How can I help you today?`;
