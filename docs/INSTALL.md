...
- `agentcall-desktop-1.0.1-amd64.deb`;
- `agentcall-desktop-1.0.1-x64-setup.exe`;
...
Use `AgentCall-1.0.1-333.apk` for UI/development validation without protected telephony-audio permissions.
...
Use `AgentCall-privileged-1.0.1-333-magisk.zip` for the qualified rooted-device path. The module already embeds the exact matched APK and protected-permission allowlist.
...
Confirm the expected package version `1.0.1 (333)`, requested/granted privileges, launcher activity and system package path. If Package Manager metadata disagrees with the mounted APK, stop; do not test calls.
