...
For the current release candidate, the only accepted identity is `com.callagent.gateway` version `2.8.54` (`332`). The debug APK and its matched module are `QUALIFICATION_ONLY_DEBUG_SIGNED`, `Publishable: NO`; they must not be described or uploaded as production-signed artifacts.
