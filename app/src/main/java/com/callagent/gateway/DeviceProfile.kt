 ...
 *
 * Historical mixer metadata is retained for device-port documentation only;
 * production never executes it. Control names and default volumes remain
...
data class DeviceProfile(
    ...
    // ── Historical mixer metadata (not executed in production) ──
    ...
    /** Shell command to enable incall_music after AudioTrack starts and the route settles. */
    ...
    // ── Phase-0 qualification metadata (read-only capability facts) ──

    /**
     * Known capability metadata for this profile.  Phase-0 profiles expose
     * what the device policy/firmware advertises WITHOUT claiming injection
     * is verified.  Defaults to [Capabilities.unset] for legacy profiles
     * whose capability surface has not been audited.
     */
    val capabilities: Capabilities = Capabilities.unset(),
) {
    companion object {
        private const val TAG = "DeviceProfile"

        /** Auto-detect the device and return the appropriate profile. */
        fun detect(evidenceProvider: ApprovedDeviceEvidenceProvider): DeviceProfile {
            Log.i(TAG, "Detecting device: hw=${Build.HARDWARE} board=${Build.BOARD} model=${Build.MODEL} device=${Build.DEVICE}")
        ...
        /** Legacy/general profile selection. Exact gram authorization requires the full overload. */
        fun selectProfile(hw: String, board: String, model: String, device: String = ""): DeviceProfile =
            selectProfile(hw, board, model, device, -1, "", "")

        /** Pure exact-tuple profile selection used by production detection. */
        fun selectProfile(
            hw: String,
            board: String,
            model: String,
            ...
            evidence: ApprovedDeviceEvidence? = null,
        ): DeviceProfile {
            return when (DeviceSelector.select(
                DeviceSelector.Identity(hw, board, model, device, apiLevel, fingerprint, vendorFingerprint),
                evidence,
            ).profileId) {
                DeviceSelector.ProfileId.ATOLL_GRAM -> atollGram()
                DeviceSelector.ProfileId.MSM8930 -> msm8930()
                DeviceSelector.ProfileId.EXYNOS9820 -> exynos9820()
                DeviceSelector.ProfileId.GENERIC_QUALCOMM -> genericQualcomm()
                DeviceSelector.ProfileId.GENERIC_EXYNOS -> genericExynos()
                DeviceSelector.ProfileId.GENERIC -> generic()

            }
        }

        private fun runtimeVendorFingerprint(): String = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            Build.getFingerprintedPartitions().firstOrNull { it.name == "vendor" }?.fingerprint.orEmpty()
        } else ""
         ...
         *    This replaces physical mic with host-agent audio in modem TX.
        ...
        /**
         * POCO M2 Pro — Qualcomm atoll / WCD937x, Android 15 (API 35).
         *
         * Phase-0 SAFE qualification profile.  The device's audio policy
         * exposes incall_music_uplink (AUDIO_OUTPUT_FLAG_INCALL_MUSIC →
         * TELEPHONY_TX) and voice_rx (TELEPHONY_RX), and the active-call
         * mixer uses VoiceMMode2 (RX_CDC_DMA_RX_0_Voice Mixer VoiceMMode2,
         * VoiceMMode2_Tx Mixer TX_CDC_DMA_TX_3_MMode2).  Incall_Music
         * Audio Mixer MultiMedia1/2/4/5/9 exist and stay OFF during an
         * ordinary call.
         *
         * None of that proves injection works yet.  So this profile is
         * deliberately inert: no mixer writes, no unverified HAL parameter,
         * no claim that VOICE_DOWNLINK returns real audio.  It exposes the
         * known capability metadata + a read-only diagnostic grep so a human
         * can qualify the device before any destructive command is enabled.
         */
        fun atollGram() = DeviceProfile(
            name = "Qualcomm atoll (POCO M2 Pro / gram)",
            // Qualified path: no destructive mixer writes. Production retains
            // these empty fields as explicit porting metadata and never executes
            // mixer commands. Muting Voice Rx / forcing Incall_Music on by guess
            // is explicitly forbidden until injection is verified.
            mixerSetupCmd = "",
            mixerRestoreCmd = "",
            mixerIncallMusicCmd = "",
            // Read-only diagnostics only: dump the voice/incall mixer state.
            // No value arguments are passed to tinymix, so nothing is written.
            mixerDiagGrep = "tinymix 2>&1 | grep -iE '(VoiceMMode|Incall_Music|RX_CDC_DMA|TX_CDC_DMA|MultiMedia[0-9])'",
            musicVolPercent = 20,
            captureGain = 1,
            playbackGain = 2,
            noiseGateThreshold = 300,
            ...
            doubleTalkRatio = 1.5f,
            requireSpeakerMode = true,
            // Unverified on this firmware — empty param makes every
            // AudioManager.setParameters() call site skip.  Do not set an
            // unsupported HAL parameter blindly.
            incallMusicParam = "",
            voiceDownlinkWorks = true,
            routeChangeDelayMs = 500,
            appopsPropagationMs = 300,
...
/**
 * Read-only capability metadata for a [DeviceProfile].  This records what the
 * device's audio policy / firmware advertises, without asserting that any
 * injection or capture path is proven.  Phase-0 profiles use this to expose
 * known facts for human qualification rather than acting on guesses.
 */
data class Capabilities(
    /** SoC/platform family, e.g. "atoll". */
    val platform: String,
    /** Audio codec family, e.g. "WCD937x". */
    val codec: String,
    /** Policy exposes incall_music_uplink (INCALL_MUSIC flag → TELEPHONY_TX). */
    val incallMusicUplinkExposed: Boolean,
    /** Policy exposes voice_rx (TELEPHONY_RX). */
    val voiceRxExposed: Boolean,
    /** Has incall_music uplink injection been verified end-to-end? */
    val injectionVerified: Boolean,
    /** Is this profile permitted to write ALSA mixer controls? */
    val mixerWritesAllowed: Boolean,
) {
    companion object {
        /** Sentinel for legacy profiles whose capability surface is unaudited. */
        fun unset() = Capabilities(
            platform = "unknown",
            codec = "unknown",
            incallMusicUplinkExposed = false,
            voiceRxExposed = false,
